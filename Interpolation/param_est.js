...
function calc_linLSQ_poly(data,order) {
  ...
  */
  ...
  /*let AT=numeric.transpose(A);
  let ATA=numeric.dot(AT,A);
  let ATAI=numeric.inv(ATA)
  let ATAIA=numeric.dot(ATAI,AT);
  let p=numeric.dot(ATAIA,b);*/
...
function calc_nonlinLSQ_gaussnewton(data,initial_p,max_iterations) {
  ...
  for(let iter=0;iter<=max_iterations;++iter) {
    ...
    */
    ...
    let JTb=numeric.dot(JT,dy);
    let JTJ=numeric.dot(JT,J);
    let dp=numeric.solve(JTJ,JTb);
...
function calc_nonlinLSQ_gradientdescent(data,initial_p,max_iterations,learning_rate) {
  ...
  for(let iter=0;iter<=max_iterations;++iter) {
    ...
    */
    let J=calc_jacobian(data,p);
    let JT=numeric.transpose(J);
    let JTy=numeric.dot(JT,dy);
    let grad=numeric.mul(2,JTy);
    ...
    * See slide 23.
    */
    grad=numeric.mul(learning_rate,grad);
